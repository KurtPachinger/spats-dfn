...
Example.bridge = function() {
	...
	var mult = document.getElementsByTagName('html')[0].className.indexOf('MS') > 0 ? 6 : 1; 
...
var sto = {
		...
		stat: {
			...
			item: ["sweetgum", "branch", "holly", "cherries", "pinecone", "leaf", "egg", /*"apple", "peach",*/ "snake", /*"ladybug",*/ "firefly", /*"acorn",*/ "disc", "boomerang", "baseball", "football", "honeybee", "rock", "icicle", "flower", /*"star",*/ "dewdrop", "vine", "spaceship", "plane", "balloon", /*"heart", "regrow", "shock"*/],
			//
			power: 100,
			score: 0,
			shots: 4
		...
		char: {
			...
			spats: "data:image/svg+xml;utf8," + encodeURIComponent(document.getElementById("spats").outerHTML),
	...
	function wave() {
		...
		sto.time[0] = setTimeout(function() {
			...
			for (var i = 0; i <= sto.curr[1]; i++) {
				...
				$("#eyelidTop").attr("transform", "translate(0 " + Math.floor(Math.random() * 3 + 1) * 1.5 + ")");
				$("#eyelidBottom").attr("transform", "translate(0 " + Math.floor(Math.random() * 2 + 1) * -3 + ")");
				$("#mouth-2, #tooth").attr("transform", "translate(0 " + 30 * movX + ")");
				$("#legL").attr("transform", "translate(0 " + (-2 + 40 * movX * -1) + ")");
				$("#legR").attr("transform", "translate(0 " + (-2 + 40 * movX) + ")");
				
				spats.render.sprite.texture = "data:image/svg+xml;utf8," + encodeURIComponent(document.getElementById("spats").outerHTML);
			} //loop no. enemies
	...
	var spats = Bodies.rectangle(320, 360, 48, 74, {
		...
		render: {
			fillStyle: "#666",
			sprite: { texture: sto.char.spats, xScale: (0.8*mult), yScale: (0.8*mult) }
		},
		collisionFilter: {
			group: 0,
			collidesWith: [0]
